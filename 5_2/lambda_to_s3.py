...
def lambda_handler(event, context):
    ...
    csv_res = extract_data(response.text, symbol)
    
    upload_to_s3(f'/tmp/{symbol}.csv','2hafta',f'bist/{symbol}.csv')
...
# url = f'https://finans.mynet.com/borsa/hisseler/acsel-acipayam-seluloz/'
# response = requests.get(url)
# response.raise_for_status()  # Ensure we got a successful response

# csv_res = extract_data(response.text, 'acsel')
